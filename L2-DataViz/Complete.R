...
#basic scatter plot
g <- ggplot(data = iris, aes(Sepal.Length, Sepal.Width, color=factor(Species))) + geom_point()

#lets get a title and axis labels in the mix
g <- g + labs(x="Length", y="Width", title="Sepal Length vs Sepal Width")
...
g <- g + guides(color = guide_legend(override.aes = list(size = 10)))
